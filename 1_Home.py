...
DEFAULT_PDFS = [
    "Constitution_Amendment.pdf", "constitution_of_india.pdf",
...
def get_pdf_text(pdf_docs):
    ...
    for pdf in pdf_docs:
        pdf_reader = PdfReader(
            BytesIO(pdf))  # Use BytesIO to handle bytes objects
    ...
    for attempt in range(retries):
        try:
            ...
            if "Rate limit" in str(e) or "quota exceeded" in str(e):
                print(
                    f"Rate limit exceeded. Attempt {attempt + 1} of {retries}."
                )
...
def get_conversational_chain():
    ...
    "I didn't find any relevant info in the pdf, then provide an answer from your side, search the internet for the answer in detail, only and only from indian constitution, and also
...
def main():
    st.set_page_config(page_title="Home", page_icon="logo.png")
    ...
    if not os.path.exists("faiss_index"):
        with st.spinner("Processing default PDFs..."):
            try:
                default_text = get_pdf_text(
                    [open(pdf, 'rb').read() for pdf in DEFAULT_PDFS])
    ...
    for message in st.session_state.messages:
        with st.chat_message(message["role"], avatar="./pictures/bot.png"):
    ...
    if st.session_state.messages[-1]["role"] != "assistant":
        with st.chat_message("assistant", avatar="./pictures/bot.png"):
